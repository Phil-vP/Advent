...
fn _both(input_line: &String, window_size: usize) {
    ...
    for window in input_line.chars().collect::<Vec<char>>().windows(window_size) {

        let unique_count = window.into_iter().unique().count();
        if unique_count == window_size {
            println!("Found marker ({}) for window size {} at counter {}", window.into_iter().collect::<String>(), window_size, counter);
            return;
        }
        counter += 1;
    }
